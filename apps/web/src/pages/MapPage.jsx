...
export default function MapPage() {
  ...
  const searchStats = useMemo(() => {
    ...
    filteredUnidades.forEach(unidade => {
      if (normalizeText(unidade.nome).includes(textNormalized)) byName++
      if (normalizeText(unidade.bairro).includes(textNormalized)) byBairro++
      if (unidade.especialidades?.some(esp => normalizeText(esp.nome).includes(textNormalized))) {
        byEspecialidade++
      }
      if ((textNormalized.includes('vacina') || textNormalized.includes('sala')) && unidade.sala_vacina) {
        bySalaVacina++
      }
    })
            ...
            {selectedUnidade ? (
              <div style={{ height: '100%' }}>
                ...
                <div style={{ padding: '24px' }}>
                  ...
                  {/* Direção da Unidade (Diretor e Diretor Adjunto) */}
                  {(selectedUnidade.diretor_responsavel || selectedUnidade.diretor_adjunto) && (
                    <div style={{ marginBottom: '16px' }}>
                      {selectedUnidade.diretor_responsavel && (
                        <div style={{ marginBottom: '8px' }}>
                          ...
                          <div style={{ fontWeight: '600', color: '#262626', fontSize: '15px' }}>{selectedUnidade.diretor_responsavel}</div>
                        </div>
                      )}
                      ...
                      {selectedUnidade.diretor_adjunto && (
                        <div>
                          ...
                          <div style={{ fontWeight: '600', color: '#262626', fontSize: '15px' }}>{selectedUnidade.diretor_adjunto}</div>
                        </div>
                      )}
                    </div>
                  )}
                      ...
                      {!searchText && searchType === 'bairro' && (
                        <Select
                          ...
                          filterOption={(input, option) =>
                            normalizeText(option.children).includes(normalizeText(input))
                          }
                        >
                          {bairros.map((bairro) => (
